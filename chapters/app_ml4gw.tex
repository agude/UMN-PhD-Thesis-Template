\chapter{The \texttt{ml4gw} Software Library}
\label{app:ml4gw}
\begin{refsection}
...
\texttt{ml4gw} addresses this gap by re-implementing common GW processing steps as PyTorch~\cite{paszke2019pytorch} modules, taking advantage of the natural parallelization that comes with PyTorch's batch processing, and adding the option to accelerate these steps using GPUs and other coprocessors supported by the PyTorch framework.
...
Like bilby~\cite{Ashton_2019}, \texttt{ml4gw} provides the functionality to sample from astrophysical parameter distributions, which can then be used to simulate waveforms, mimicking the simulation features of lalsuite~\cite{lalsuite}.
