\chapter{Validation of \amplfi on Simulated and Real Events}
...
We demonstrate sky localization area performance comparable to BAYESTAR, and find that \amplfi provides tighter searched volume constraints. 
...
In Chapter~\ref{chap:amplfi_methods}, we introduced \amplfi, a parameter estimation (PE) algorithm based on likelihood-free inference (LFI), as a tool for providing low-latency PE data products.
...
In Sec.~\ref{amp_res_sec:review} we describe \amplfi and the improvements incorporated for this work. 
...
In Sec.~\ref{amp_res_sec:results-skyloc} we compare \amplfi's sky localization performance with BAYESTAR~\cite{Singer_2016}. 
In Sec.~\ref{amp_res_sec:results-intrinsic} we demonstrate the self-consistency of \amplfi sky localizations. 
...
\amplfi is a PE algorithm based on LFI and was presented in detail in Chapter~\ref{chap:amplfi_methods}. 
...
\begin{enumerate}
    ...
    This is done using signal generators implemented in \texttt{torch} and publicly available in the \mlgw library\footnote{\url{https://github.com/ML4GW/ml4gw}}~\cite{Benoit2025}.
...
We take advantage of an efficient out-of-memory data loader implemented in \mlgw so that data loading does not bottleneck the training process. 
This is the same data loader implementation is used to train the \aframe detection algorithm~\cite{aframe-methods}.

Lastly, in Ref.~\cite{Chatterjee:2024pbj}, \amplfi was trained with a GPU-accelerated IMRPhenomD approximant~\cite{Khan:2015jqa}. 
...
\begin{table}[ht]
    ...
    \hline
    \end{tabular}
    \caption[Neural network hyperparameters for the HL and HLV models used to analyze the O3 MDC]{Neural network hyperparameters for the HL and HLV models used to analyze the O3 MDC.}
...
\begin{figure}
    \centering
    \includegraphics[width=0.88\columnwidth]{figures/distributions.pdf}
...
See Sec.~5 of Ref.~\cite{Singer_2016} for more details. 
...
\begin{figure*}
\includegraphics[width=0.48\textwidth]{figures/amplfi_vs_bayestar_searched_area.pdf}
\includegraphics[width=0.48\textwidth]{figures/amplfi_vs_bayestar_searched_volume.pdf}
...
\begin{figure}
    ...
    \caption[Normalized histogram of cos $\delta \theta$ for the HLV injection set]{Normalized histogram of cos $\delta \theta$ for the HLV injection set, where $\delta \theta$ is the angular offset between the true sky position of the injection and the maximum a posteriori pixel.}
...
In Fig.~\ref{amp_res_fig:amplfi-vs-bayestar} we compare searched areas (top) and searched volumes (bottom) between \amplfi and BAYESTAR sky maps for both HLV and HL datasets. 
...
Fig.~\ref{amp_res_fig:self-consistency} shows self-consistency tests for the HLV \amplfi model using injections with SNR $\geq 12$. 
Consistency tests for area and volume are shown to be unbiased. 
A small bias in distance is present. 
Because \amplfi is trained directly on strain data it does not require additional pipeline-dependent tuning factors to achieve self-consistency.
...
We see that \amplfi's sky localizations remain well calibrated and accurate 11 weeks beyond its initial training period. 
...
First, \amplfi trains using a distance prior that is uniform in luminosity distance, while the GWTC-3 analyses utilize a uniform in co-moving volume distribution. 
...
For example, \amplfi does not show multi-modality in masses exhibited by the GWTC-3 result in GW200129\_065458 (see Fig.~\ref{amp_res_fig:GW200129_065458-hlv}). 
...
For GW191215\_223052 and GW200220\_124850, \amplfi does not show the multi-modality in inclination shown by the GWTC-3 result (see~\Cref{amp_res_fig:GW191215_223052-hlv,amp_res_fig:GW200220_124850-dist-railing}). 
...
With \amplfi's performance validated on O3 data, the \aframe and \amplfi algorithms could be combined into a single production pipeline.
...
\begin{figure*}
    ...
    \caption[Posterior comparison between \amplfi and GWTC-3 for GW191215\_223052]{Posterior comparison between \amplfi and GWTC-3 result for GW191215\_223052. Data from Hanford, Livingston, and Virgo are analyzed. The BAYESTAR sky map produced in low-latency is plotted in purple. Sky map contours correspond to 90\% confidence intervals.}
...
\begin{figure*}
    ...
    \caption[Posterior comparison between \amplfi and GWTC-3 for GW200129\_065458]{Posterior comparison between \amplfi and GWTC-3 result for GW200129\_065458. Data from Hanford, Livingston, and Virgo are analyzed. The BAYESTAR sky map produced in low-latency is plotted in purple. Sky map contours correspond to 90\% confidence intervals. For this event, bi-modalities in $m_1$ and $m_2$ are not captured by \amplfi. In addition, \amplfi's distance and inclination posteriors disagree with the GWTC-3 result.}
...
\begin{figure*}
    ...
    \caption[Posterior comparison between \amplfi and GWTC-3 for GW200208\_130117]{Posterior comparison between \amplfi and GWTC-3 result for GW200208\_130117. Data from Hanford, Livingston, and Virgo are analyzed. The BAYESTAR sky map produced in low-latency is plotted in purple. Sky map contours correspond to 90\% confidence intervals.}
...
\begin{figure*}
    ...
    \caption[Posterior comparison between \amplfi and GWTC-3 for GW200311\_115853]{Posterior comparison between \amplfi and GWTC-3 result for GW200311\_115853. Data from Hanford, Livingston, and Virgo are analyzed. The BAYESTAR sky map produced in low-latency is plotted in purple. Sky map contours correspond to 90\% confidence intervals.}
...
\begin{figure*}
    ...
    \caption[Posterior comparison between \amplfi and GWTC-3 for GW200224\_222234]{Posterior comparison between \amplfi and GWTC-3 result for GW200224\_222234. Data from Hanford, Livingston, and Virgo are analyzed. The BAYESTAR sky map produced in low-latency is plotted in purple. Sky map contours correspond to 90\% confidence intervals.}
...
\begin{figure*}
    ...
    \caption[Posterior comparison between \amplfi and GWTC-3 for GW200225\_060421]{Posterior comparison between \amplfi and GWTC-3 result for GW200225\_060421. Data from Hanford and Livingston are analyzed. The BAYESTAR sky map produced in low-latency is plotted in purple. Sky map contours correspond to 90\% confidence intervals.}
...
\begin{figure*}
    ...
    \caption[Posterior comparison between \amplfi and GWTC-3 for GW191204\_110529]{Posterior comparison between \amplfi and GWTC-3 result for GW191204\_110529. Data from Hanford and Livingston are analyzed. Sky map contours correspond to 90\% confidence intervals.}
...
\begin{figure*}
    ...
    \caption[Posterior comparison between \amplfi and GWTC-3 for GW200306\_093714]{Posterior comparison between \amplfi and GWTC-3 result for GW200306\_093714. Data from Hanford and Livingston are analyzed. Sky map contours correspond to 90\% confidence intervals.}
...
\begin{figure*}
    ...
    \caption[Posterior comparison between \amplfi and GWTC-3 for GW200220\_124850]{Posterior comparison between \amplfi and GWTC-3 result for GW200220\_124850. Data from Hanford and Livingston are analyzed. This illustrates the scenario where the distance posterior has support well beyond \amplfi's training prior. Still, \amplfi is able to recover source parameters and localizations in rough agreement with the GWTC-3 result.}
...
\begin{figure*}
    ...
    \caption[Posterior comparison between \amplfi and GWTC-3 for GW200128\_022011]{Posterior comparison between \amplfi and GWTC-3 result for GW200128\_022011. Data from Hanford and Livingston are analyzed. The BAYESTAR sky map produced in low-latency is plotted in purple. This illustrates the scenario where the distance posterior has support well beyond \amplfi's training prior. Still, \amplfi is able to recover source parameters and localizations in rough agreement with the GWTC-3 result.}
...
\begin{figure*}
    ...
    \caption[Posterior comparison between \amplfi and GWTC-3 for GW191222\_033537]{Posterior comparison between \amplfi and GWTC-3 result for GW191222\_033537. Data from Hanford and Livingston are analyzed. The BAYESTAR sky map produced in low-latency is plotted in purple. This illustrates the scenario where the distance posterior has support well beyond \amplfi's training prior. Still, \amplfi is able to recover source parameters and localizations in rough agreement with the GWTC-3 result.}
