...
In Figure~\ref{amp_res_fig:amplfi-vs-bayestar} we compare searched areas (top) and searched volumes (bottom) between \texttt{AMPLFI} and BAYESTAR sky maps for both HLV and HL datasets. 
...
In Figure~\ref{amp_res_fig:dtheta} we compare the angular offset between the sky map's maximum a posteriori pixel and the true pixel of the injection. 
...
Figure~\ref{amp_res_fig:self-consistency} shows self-consistency tests for the HLV \texttt{AMPLFI} model using injections with SNR $\geq 12$. 
...
For example, \texttt{AMPLFI} does not show multi-modality in masses exhibited by the GWTC-3 result in GW200129\_065458 (see Figure~\ref{amp_res_fig:GW200129_065458-hlv}). 
