...
Here, we report validation studies of \texttt{AMPLFI}, a likelihood-free inference solution to low-latency parameter estimation of binary black holes. 
...
We demonstrate sky localization area performance comparable to BAYESTAR, and find that \texttt{AMPLFI} provides tighter searched volume constraints. 
We show accurate reconstruction of source parameters with uncertainties for use in distributing low-latency coarse-grained chirp mass information. In addition, we analyze several candidate events reported by the LVK in the third gravitational-wave transient catalog (GWTC-3) and show consistency with the LVK's analysis. 
Altogether, we demonstrate \texttt{AMPLFI}'s ability to produce data products for low-latency public alerts.


\section{Overview}  \label{amp_res_sec:intro}
A decade has passed since the first direct detection of gravitational waves (GWs)~\cite{AbEA2016a}. 
...
The discovery rate is expected to continue to rise as the Advanced LIGO~\cite{Aasi_2015}, Advanced Virgo~\cite{Acernese_2015} and KAGRA~\cite{kagra} interferometers approach the design sensitivity era of the fifth observing run~\cite{observing_scenarios}. 
...
For detection, many methods are being developed, showing promise in achieving the sensitivity of traditional matched-filter pipelines at a reduced latency and computational cost~\cite{aframe-methods, sage, nousi2022deep}. 
...
For example, the production sky localization algorithm for events from matched-filter pipelines, BAYESTAR, requires signal to noise ratio (SNR) time series~\cite{Singer_2016}. 
...
In Chapter~\ref{chap:amplfi_methods}, we introduced \texttt{AMPLFI}, a parameter estimation (PE) algorithm based on likelihood-free inference (LFI), as a tool for providing low-latency PE data products.
We motivated \texttt{AMPLFI} as a real-time followup to ML-based detection algorithms, specifically, \texttt{Aframe}~\cite{aframe-methods}.
In this paper, we describe recent improvements to \texttt{AMPLFI}, and demonstrate its ability to produce robust low-latency source PE data products. 

This paper is organized as follows. 
In Sec.~\ref{amp_res_sec:lfi} we briefly summarize likelihood-free inference.
In Sec.~\ref{amp_res_sec:review} we describe \texttt{AMPLFI} and the improvements incorporated for this work. 
In Sec.~\ref{amp_res_sec:testing-data} we present the dataset used to evaluate \texttt{AMPLFI}'s performance. 
In Sec.~\ref{amp_res_sec:sky-localization}, we discuss and benchmark our method for producing sky localizations from \texttt{AMPLFI} posterior samples. 
In Sec.~\ref{amp_res_sec:results-skyloc} we compare \texttt{AMPLFI}'s sky localization performance with BAYESTAR~\cite{Singer_2016}. 
In Sec.~\ref{amp_res_sec:results-intrinsic} we demonstrate the self-consistency of \texttt{AMPLFI} sky localizations. 
In Sec.~\ref{amp_res_sec:results-robustness}, we illustrate \texttt{AMPLFI}'s robustness to changing detector background over a time scale of several months. 
...
Traditional Bayesian PE algorithms rely on repeated evaluations of an explicit likelihood function, utilizing stochastic sampling techniques such as Markov Chain Monte Carlo (MCMC) or nested sampling to explore the signal parameter space~\cite{Ashton_2019, lalinference_paper}. 
...
Low-latency PE methods typically make approximations, to either the likelihood function or waveform physics, to simplify the exploration of the posterior~\cite{Canizares:2014fya, Morisaki:2023kuq, Morisaki:2020oqk, Singer_2016}.
...
\section{\texttt{AMPLFI}} \label{amp_res_sec:review}
\texttt{AMPLFI} is a PE algorithm based on LFI and was presented in detail in Chapter~\ref{chap:amplfi_methods}. 
...
While the core principle of \texttt{AMPLFI} is similar, there are significant implementation differences, namely
...
\begin{itemize}
    ...
    \item Real-time data processing steps like filtering, power spectral density (PSD) estimation, and whitening utilizing the GPU.
\end{itemize}
...
\begin{enumerate}
    ...
    This is done using signal generators implemented in \texttt{torch} and publicly available in the \texttt{ml4gw} library\footnote{\url{https://github.com/ML4GW/ml4gw}}~\cite{Benoit2025}.
...
In Ref.~\cite{Chatterjee:2024pbj}, \texttt{AMPLFI} was trained and evaluated using a fraction of a day of strain data. 
...
We take advantage of an efficient out-of-memory data loader implemented in \texttt{ml4gw} so that data loading does not bottleneck the training process. 
This is the same data loader implementation is used to train the \texttt{Aframe} detection algorithm~\cite{aframe-methods}.

Lastly, in Ref.~\cite{Chatterjee:2024pbj}, \texttt{AMPLFI} was trained with a GPU-accelerated IMRPhenomD approximant~\cite{Khan:2015jqa}. 
Since then, we have implemented the IMRPhenomPv2 waveform approximant, allowing \texttt{AMPLFI} to be trained with a waveform that includes spin precession physics.

\subsubsection{Neural Network}
In Ref.~\cite{Chatterjee:2024pbj}, the \texttt{AMPLFI} embedding network $\Gamma_\psi$ consisted of a 1-dimensional convolutional ResNet~\cite{resnet}, which processed time-domain representations of the data to produce a lower-dimensional data summary. 
...
Multi-modal learning approaches have been explored in other GW data analysis contexts~\cite{PhysRevD.108.042004, Sun:2023vlq}.
...
For the studies that follow, we train two \texttt{AMPLFI} models for Hanford-Livingston (HL) and Hanford-Livingston-Virgo (HLV) detector configurations. 
...
In preparation for O4, Ref.~\cite{Chaudhary_2024} performed a study which added simulated CBC signals (known colloquially as \emph{injections}) into a real-time data replay. 
The injections were added across a one month period during O3b, between 2020-01-05T23:59:42 and 2020-02-14T23:59:42. 
Search pipelines analyzed the data replay using their online configurations. GWCelery\footnote{\url{https://rtd.igwn.org/projects/gwcelery/en/latest/}} processed pipeline triggers, mimicking the end-to-end real-time alert infrastructure. 
...
We first filter the injections to those consistent with \texttt{AMPLFI}'s training prior (see Table~\ref{amp_res_tab:priors}). 
...
BAYESTAR is configured to use the same distance prior used to train \texttt{AMPLFI}. After these cuts, this leaves 1233 HL and 903 HLV injections. 
Fig.~\ref{amp_res_fig:mdc_dist} visualizes the distribution of signal parameters. 
...
\hline
\end{tabular}
\caption[Prior distributions for source parameters used to generate waveforms during \texttt{AMPLFI} training]{Prior distributions for source parameters used to generate waveforms during training. 
Parameters which \texttt{AMPLFI} is trained to infer are specified. We use a spin parameterization consistent with Bilby. Note that the distance prior is uniform in distance. 
...
Using the dataset of injections described in Sec.~\ref{amp_res_sec:testing-data}, we compare the accuracy of the \texttt{AMPLFI} sky map produced using the histogram and KDE estimators. 
...
The top row of Fig.~\ref{amp_res_fig:kde-vs-hist} shows that searched areas from the two methods are in statistical agreement.
...
In this regime, the histogram method's searched volume is comparable with the KDE. 
...
\begin{figure} 
    ...
    Error bars correspond to 1$\sigma$ confidence intervals calculated by bootstrapping. 
...
\caption[Comparison between searched areas and volumes for \texttt{AMPLFI} and BAYESTAR]{Top Left: Scatter plot comparing searched areas of \texttt{AMPLFI} and BAYESTAR. 
...
Bottom: Same as top, but for searched volumes. 
\texttt{AMPLFI} provides smaller searched volumes than BAYESTAR} \label{amp_res_fig:amplfi-vs-bayestar}
\end{figure*} 
...
We compare \texttt{AMPLFI}'s sky localization performance with BAYESTAR using the dataset set described in Sec.~\ref{amp_res_sec:testing-data}. 
In Fig.~\ref{amp_res_fig:amplfi-vs-bayestar} we compare searched areas (top) and searched volumes (bottom) between \texttt{AMPLFI} and BAYESTAR sky maps for both HLV and HL datasets. 
Bootstrapped confidence intervals show the searched area cumulative histograms agree within 2$\sigma$. 
The searched volume cumulative histogram shows that \texttt{AMPLFI} provides smaller searched volumes than BAYESTAR. 
One drawback of searched area and volume as performance metrics is that they do not capture sky map multi-modality (see Ref.~\cite{Essick:2014wwa} for a discussion).
In Fig.~\ref{amp_res_fig:dtheta} we compare the angular offset between the sky map's maximum a posteriori pixel and the true pixel of the injection. 
We plot $\cos \delta \theta$ which highlights groupings at the true and antipodal location. 
We see agreement between the BAYESTAR and \texttt{AMPLFI} distributions.
...
For BAYESTAR, this calibration is achieved by rescaling the SNR timeseries by a correction factor which is tuned to injections~\cite{Singer_2016}. 
...
Using this default correction factor, BAYESTAR sky maps produced from the SNR timeseries provided by the PyCBC live analysis have been shown to be biased (see Fig. 5 in Ref.~\cite{Nitz_2018}). 
...
Fig.~\ref{amp_res_fig:self-consistency} shows self-consistency tests for the HLV \texttt{AMPLFI} model using injections with SNR $\geq 12$. 
Consistency tests for area and volume are shown to be unbiased. 
A small bias in distance is present. 
Because \texttt{AMPLFI} is trained directly on strain data it does not require additional pipeline-dependent tuning factors to achieve self-consistency.

\subsection{Intrinsic Parameter Recovery}
\label{amp_res_sec:results-intrinsic}
...
Because \texttt{AMPLFI} provides a full posterior, estimates of uncertainty can be used in low latency. 
...
In Fig.~\ref{amp_res_fig:chirp-mass-recovery}, we illustrate \texttt{AMPLFI}'s ability to recover the chirp mass of the source. 
We see the expected increase in uncertainty as the injected chirp mass increases. 
In addition, we perform a P-P test using 500 injections drawn from \texttt{AMPLFI}'s training prior. 
Fig.~\ref{amp_res_fig:pp-plot} shows a P-P test demonstrating \texttt{AMPLFI}'s unbiased recovery of parameters.

\begin{figure*}
    \centering
    \includegraphics[width=\textwidth]{figures/skymap_consistency.pdf}
    \caption[P-P tests for \texttt{AMPLFI}'s joint 2D (area) and 3D (volume) posterior distributions measuring sky map self consistency]{P-P tests for \texttt{AMPLFI}'s joint 2D (area) and 3D (volume) posterior distributions measuring sky map self consistency. 
...
\begin{figure}
    ...
    \caption[Scatter plot comparing true chirp mass and \texttt{AMPLFI}'s median chirp mass for simulated signals from the injection dataset]{Scatter plot comparing true chirp mass and \texttt{AMPLFI}'s median chirp mass for simulated signals from the injection dataset described in Sec.~\ref{amp_res_sec:testing-data}. Error bars corresponding to the $5^{\mathrm{th}}$ and $95^{\mathrm{th}}$ percentiles of \texttt{AMPLFI}'s posterior samples are shown. Orange and blue points correspond to the HL and HLV networks respectively.} 
...
\begin{figure*}
    ...
    \caption[P-P tests for \texttt{AMPLFI}'s 1 dimensional marginal posterior distributions]{P-P tests for \texttt{AMPLFI}'s 1 dimensional marginal posterior distributions. 
    500 injections from \texttt{AMPLFI}'s training prior were analyzed. 
...
As described in Sec~\ref{amp_res_sec:review}, \texttt{AMPLFI} utilizes real detector strain data for training, and so our noise model is constructed empirically. 
...
To validate \texttt{AMPLFI}'s robustness over time, we created 4 datasets of 1000 injections sampled from \texttt{AMPLFI}'s training prior and added them into real detector noise. 
...
We analyze each of these datasets using the HLV \texttt{AMPLFI} network. 
In Fig~\ref{amp_res_fig:robustness}, we show sky localization consistency and searched area performance. 
We see that \texttt{AMPLFI}'s sky localizations remain well calibrated and accurate 11 weeks beyond its initial training period. 
This demonstrates that a single \texttt{AMPLFI} model maintains utility well beyond its initial training period. 

\begin{figure*}
    \centering
    \includegraphics[width=\textwidth]{figures/performance_over_time_consistency.pdf}
    \caption[Left: 2D (area) localization P-P tests for 4 datasets of injections drawn from \texttt{AMPLFI}'s training prior created at various epochs across O3b. 
    Right: Cumulative histogram of searched area for the same datasets.]{Left: 2D (area) localization P-P tests for 4 datasets of injections drawn from \texttt{AMPLFI}'s training prior created at various epochs across O3b. 
    Analyzed injections are restricted to have SNR $\geq 12$. 
    P-values shown in the legend indicate that sky maps produced by \texttt{AMPLFI} remain well calibrated across an 11 week period. 
    ...
    The accuracy of \texttt{AMPLFI} sky maps does not degrade over time.}
...
We analyze those candidates with posterior support within \texttt{AMPLFI}'s training prior (Table \ref{amp_res_tab:priors}), and which occurred during or after the period of data used to train \texttt{AMPLFI}. 
...
For several candidates, source mass posteriors from the GWTC-3 analysis were completely within \texttt{AMPLFI}'s training prior, but luminosity distance support only partially overlapped. 
We analyze these events, but note that \texttt{AMPLFI}'s luminosity distance posterior rails against the prior. 
Still, \texttt{AMPLFI}'s sky maps and posterior estimates for source parameters are in rough agreement with the LVK analysis. 
...
A possible solution to this problem would be to train \texttt{AMPLFI} to infer the chirp distance ($d_{\mathrm{chirp}} \propto d_L/\mathcal{M}^{5/6}$)~\cite{Roulet_2022, Brady_2008} instead of the luminosity distance. 
...
There exist several differences between the GWTC-3 and \texttt{AMPLFI} analyses. 
First, \texttt{AMPLFI} trains using a distance prior that is uniform in luminosity distance, while the GWTC-3 analyses utilize a uniform in co-moving volume distribution. 
To account for this, we use importance sampling to re-weight \texttt{AMPLFI}'s samples to the uniform in co-moving volume distance distribution reported by the GWTC-3 analysis. 
Second, \texttt{AMPLFI} was trained using the IMRPhenomPV2 waveform, while the LVK posteriors were produced using IMRPhenomXPHM. 
So, effects of higher order modes are not captured in \texttt{AMPLFI}'s results. 
Third, the \texttt{AMPLFI} result was produced using the low-latency data, while the GWTC-3 result utilized data that includes an update to calibration, and cleaning of certain noise sources. 
Lastly, \texttt{AMPLFI} does not aim to explicitly model the Gaussian likelihood typically assumed in CBC PE. 
...
For example, \texttt{AMPLFI} does not show multi-modality in masses exhibited by the GWTC-3 result in GW200129\_065458 (see Fig.~\ref{amp_res_fig:GW200129_065458-hlv}). 
For this same event, \texttt{AMPLFI}'s distance and inclination estimates disagree with the GWTC-3 result. 
With an SNR of $\sim$ 27, waveform systematics become more important and could explain this discrepancy. 
For GW191215\_223052 and GW200220\_124850, \texttt{AMPLFI} does not show the multi-modality in inclination shown by the GWTC-3 result (see~\Cref{amp_res_fig:GW191215_223052-hlv,amp_res_fig:GW200220_124850-dist-railing}). 
Differences between the \texttt{AMPLFI} and GWTC-3 results may stem from two qualitatively distinct sources. 
First, there are differences in the underlying physical assumptions of the likelihood, such as the choice of waveform physics (for example, the inclusion of higher-order modes in GWTC-3) and the treatment of noise statistics (Gaussian noise in GWTC-3 versus real detector noise in \texttt{AMPLFI}). 
...
\section{Summary}
We have presented the performance of the \texttt{AMPLFI} algorithm across several metrics. 
We have demonstrated \texttt{AMPLFI}'s sky localization performance is equivalent to BAYESTAR using injections from an online data replay. 
We have shown that a single \texttt{AMPLFI} algorithm trained using $\sim 2$ months of real data maintains performance months beyond its training period. 
Finally, we analyzed real gravitational wave candidates within \texttt{AMPLFI}'s training prior and have shown posterior results in rough agreement with the GWTC-3 analyses. 

Still, there remains several avenues for improving the utility of \texttt{AMPLFI}.
Broadening the parameter space to include neutron star black hole and binary neutron star (BNS) mergers will be critical for analyzing more likely candidates of EM emission. 
...
For example, training \texttt{AMPLFI} to infer chirp distance~\cite{Brady_2008} instead of luminosity distance could help alleviate the distance prior railing exhibited in \texttt{AMPLFI}'s analysis of several real candidates.

With \texttt{AMPLFI}'s performance validated on O3 data, the \texttt{Aframe} and \texttt{AMPLFI} algorithms could be combined into a single production pipeline.
Chapter~\ref{chap:online} describes the software architecture used to deploy this system in real time during O4c.
...
\begin{figure*}
    ...
    \caption[Posterior comparison between \texttt{AMPLFI} and GWTC-3 for GW191215\_223052]{Posterior comparison between \texttt{AMPLFI} and GWTC-3 result for GW191215\_223052. Hanford, Livingston and Virgo data is analyzed. The BAYESTAR sky map produced in low-latency is plotted in purple. Sky map contours correspond to 90\% confidence intervals.}
...
\begin{figure*}
    ...
    \caption[Posterior comparison between \texttt{AMPLFI} and GWTC-3 for GW200129\_065458]{Posterior comparison between \texttt{AMPLFI} and GWTC-3 result for GW200129\_065458. Hanford, Livingston and Virgo data is analyzed. The BAYESTAR sky map produced in low-latency is plotted in purple. Sky map contours correspond to 90\% confidence intervals. For this event, bi-modalities in $m_1$ and $m_2$ are not captured by \texttt{AMPLFI}. In addition, \texttt{AMPLFI}'s distance and inclination posteriors disagree with the GWTC-3 result.}
...
\begin{figure*}
    ...
    \caption[Posterior comparison between \texttt{AMPLFI} and GWTC-3 for GW200208\_130117]{Posterior comparison between \texttt{AMPLFI} and GWTC-3 result for GW200208\_130117. Hanford, Livingston and Virgo data is analyzed. The BAYESTAR sky map produced in low-latency is plotted in purple. Sky map contours correspond to 90\% confidence intervals.}
...
\begin{figure*}
    ...
    \caption[Posterior comparison between \texttt{AMPLFI} and GWTC-3 for GW200311\_115853]{Posterior comparison between \texttt{AMPLFI} and GWTC-3 result for GW200311\_115853. Hanford, Livingston and Virgo data is analyzed. The BAYESTAR sky map produced in low-latency is plotted in purple. Sky map contours correspond to 90\% confidence intervals.}
...
\begin{figure*}
    ...
    \caption[Posterior comparison between \texttt{AMPLFI} and GWTC-3 for GW200224\_222234]{Posterior comparison between \texttt{AMPLFI} and GWTC-3 result for GW200224\_222234. Hanford, Livingston and Virgo data is analyzed. The BAYESTAR sky map produced in low-latency is plotted in purple. Sky map contours correspond to 90\% confidence intervals.}
...
\begin{figure*}
    ...
    \caption[Posterior comparison between \texttt{AMPLFI} and GWTC-3 for GW200225\_060421]{Posterior comparison between \texttt{AMPLFI} and GWTC-3 result for GW200225\_060421. Hanford and Livingston data is analyzed. The BAYESTAR sky map produced in low-latency is plotted in purple. Sky map contours correspond to 90\% confidence intervals.}
...
\begin{figure*}
    ...
    \caption[Posterior comparison between \texttt{AMPLFI} and GWTC-3 for GW191204\_110529]{Posterior comparison between \texttt{AMPLFI} and GWTC-3 result for GW191204\_110529. Hanford and Livingston data is analyzed. Sky map contours correspond to 90\% confidence intervals.}
...
\begin{figure*}
    ...
    \caption[Posterior comparison between \texttt{AMPLFI} and GWTC-3 for GW200306\_093714]{Posterior comparison between \texttt{AMPLFI} and GWTC-3 result for GW200306\_093714. Hanford and Livingston data is analyzed. Sky map contours correspond to 90\% confidence intervals.}
...
\begin{figure*}
    ...
    \caption[Posterior comparison between \texttt{AMPLFI} and GWTC-3 for GW200220\_124850]{Posterior comparison between \texttt{AMPLFI} and GWTC-3 result for GW200220\_124850. Hanford and Livingston data is analyzed. This illustrates the scenario where the distance posterior has support well beyond \texttt{AMPLFI}'s training prior. Still, \texttt{AMPLFI} is able to recover source parameters and localizations in rough agreement with the GWTC-3 result.}
...
\begin{figure*}
    ...
    \caption[Posterior comparison between \texttt{AMPLFI} and GWTC-3 for GW200128\_022011]{Posterior comparison between \texttt{AMPLFI} and GWTC-3 result for GW200128\_022011. Hanford and Livingston data is analyzed. The BAYESTAR sky map produced in low-latency is plotted in purple. This illustrates the scenario where the distance posterior has support well beyond \texttt{AMPLFI}'s training prior. Still, \texttt{AMPLFI} is able to recover source parameters and localizations in rough agreement with the GWTC-3 result.}
...
\begin{figure*}
    ...
    \caption[Posterior comparison between \texttt{AMPLFI} and GWTC-3 for GW191222\_033537]{Posterior comparison between \texttt{AMPLFI} and GWTC-3 result for GW191222\_033537. Hanford and Livingston data is analyzed. The BAYESTAR sky map produced in low-latency is plotted in purple. This illustrates the scenario where the distance posterior has support well beyond \texttt{AMPLFI}'s training prior. Still, \texttt{AMPLFI} is able to recover source parameters and localizations in rough agreement with the GWTC-3 result.}
