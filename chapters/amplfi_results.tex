...
In the previous chapter, we introduced \texttt{AMPLFI}, a parameter estimation (PE) algorithm based on likelihood-free inference (LFI), as a tool for providing low-latency PE data products. 
...
In Sec.~\ref{amp_res_sec:review} we discuss the \texttt{AMPLFI} algorithm previously presented in Ref.~\cite{Chatterjee:2024pbj} including improvements made since. 
...
We see that \texttt{AMPLFI}'s sky localizations remain well calibrated and accurate 11 weeks beyond \texttt{AMPLFI}'s initial training period. 
...
First, \texttt{AMPLFI} trains using a distance prior that is uniform in Euclidean volume, while the GWTC-3 analyses utilize a uniform in co-moving volume distribution. 
...
For GW191215\_223052 and GW200220\_124850, \texttt{AMPLFI} does not show multi-modality in inclination exhibited by the GWTC-3 result (see~\Cref{amp_res_fig:GW191215_223052-hlv,amp_res_fig:GW200220_124850-dist-railing}). 
